...
import { getCustomFields, addCustomField, updateCustomField, deleteCustomField, getFormTemplates, saveFormTemplate, deleteFormTemplate, getCustomTabs, addCustomTab, deleteCustomTab, updateCustomTab } from "@/features/crm/actions";
...
export function CRMFormBuilder({ value: rawValue, onChange }: CRMFormBuilderProps) {
  ...
  const dynamicCrmDbFields = {
    ...
    ...customFields.reduce((acc, field) => {
      if (field.type === "repeater" && field.subFields && Array.isArray(field.subFields)) {
        field.subFields.forEach((sub: any) => {
          acc[`${field.id}.${sub.id}`] = `${field.label} ➔ ${sub.label} (שדה מותאם חוזר)`;
        });
      } else {
              ...
              <div className="bg-white p-6 rounded-3xl border space-y-4 text-xs">
                ...
                <div className="flex items-center gap-2 pt-2">
                  ...
                  <label htmlFor="save-to-crm-check" className="font-bold text-slate-700 cursor-pointer">
                    סנכרן ועדכן איש קשר ב-CRM של בית חב"ד באופן אוטומטי
                  </label>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="2">מזכירות בית חב"ד</option>
        ...
        <Modal.Content className="text-right font-sans max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          {crmSettingsTab === "tabs" ? (
            ...
            <div className="space-y-4 py-2 text-xs">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="space-y-1">
                    <label className="font-bold text-slate-650">שם השדה (תווית)</label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-slate-650">סוג השדה</label>
                    <select
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-slate-650">לשונית קטגוריה</label>
                    <select
              ...
              <div className="space-y-2">
                <span className="font-bold text-slate-400">שדות מותאמים אישית קיימים:</span>
                {customFields.length === 0 ? (
                  <div className="text-center py-6 text-slate-500 italic bg-white border border-dashed rounded-2xl">אין שדות מותאמים אישית. הגדר שדות חדשים בממשק למעלה.</div>
                ) : (
                  <div className="space-y-2">
                        ...
                        <div key={field.id} className="p-3 bg-white border border-slate-100 rounded-xl space-y-2">
                          ...
                          {field.type === "repeater" && field.subFields && Array.isArray(field.subFields) && (
                            <div className="bg-slate-50 p-2 rounded-lg text-[10px] text-slate-500 flex gap-2 flex-wrap">
                              <span className="font-bold text-slate-600 border-l pl-2">תתי-שדות:</span>
