...
export function ServicesGridEditor({ items, onUpdate }: ServicesGridEditorProps) {
  ...
  const handleSaveItem = () => {
    if (editingItemId === "new") {
      ...
      const updated = items.map(item => item.id === editingItemId ? { ...item, ...editForm } as ServiceItem : item);
      onUpdate(updated);
    }
  ...
  const handleToggleVisibility = (id: string) => {
    onUpdate(items.map(item => item.isVisible === undefined ? { ...item, isVisible: false } : item.id === id ? { ...item, isVisible: !item.isVisible } : item));
  };
