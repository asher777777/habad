...
export function HomeEditor({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveGlobalSettings(JSON.parse(JSON.stringify(globalSettings)));
  ...
  const handleOpenPromptModal = () => {
    setCustomImagePrompt(`A highly engaging, professional, and visually striking cover image representing a Chabad House. 
Keywords: ${config.seo?.keywords || "Jewish community, Chabad, spirituality, warmth, welcoming, events"}. 
    ...
    return (
      <div className="absolute top-4 right-4 z-50 bg-white/95 backdrop-blur-md px-6 py-2 rounded-2xl border shadow-lg flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 border-r pr-4 mr-2">
          ...
          />
          {sectionKey === "landingSection" && (
            <input 
              type="range" 
              min="0" max="100" 
              value={(config[sectionKey as keyof typeof config] as any)?.backgroundOpacity ?? 85}
              onChange={(e) => setConfig({ ...config, [sectionKey]: { ...(config[sectionKey as keyof typeof config] as any), backgroundOpacity: parseInt(e.target.value) }})}
              className="w-16 h-1 cursor-pointer ml-1"
              title="שקיפות צבע רקע"
            />
          )}
        </div>
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder='לדוגמה: בית חב"ד | המקום שלך באהבה'
                  />
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    placeholder='הפרד בפסיקים (למשל: בית חב"ד, תפילין, פעילות קהילתית)'
                  />
                </div>
          ...
          <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
            ...
            {activeAccordion === "logo" && (
              <div className="p-5 bg-white space-y-5 animate-in fade-in duration-200">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-600 block">אייקון כרטיסיית דפדפן (Favicon)</label>
                  <p className="text-[10px] text-slate-500 mb-1">מומלץ תמונה ריבועית בגודל 192x192 או 512x512</p>
                  <div className="p-2 border border-slate-100 rounded-xl bg-slate-50/30">
                    <ImageUpload 
                      currentImage={globalSettings.siteFaviconUrl || ""}
                      onSelect={(url) => setGlobalSettings({ ...globalSettings, siteFaviconUrl: url })}
                      preserveFormat={true}
                    />
                  </div>
                </div>
                              ...
                              >
                                <optgroup label="עמודים">
                                  ...
                                  {sitePages.map((page, i) => (
                                    <option key={`${page.id}-${i}`} value={page.url}>
                                      {page.title} ({page.url})
                                    </option>
                                  ))}
                                </optgroup>
